...
@implementation AgentX509Request
...
- (id) initWithData:(NSData *) requestData
{
    if (self = [super init]) {
 	...
 	req = d2i_X509_REQ(0, &bytes, [requestData length]);
    }
    return self;
}
...
- (id) initWithText:(NSString *) string
{
    if (self = [super init]) {
        ...
        if(BIO_write(buffer,
                     ...
                     (int) [string length]) != [string length])
            NSLog(@"error feeding buffer");
        if(PEM_read_bio_X509_REQ(buffer,&req,0,NULL)) {
...
- (void) dump 
{
  ...
  if (attr_count>0)
     fprintf(stderr,"Attributes present: %i \n", attr_count);
  else
     fprintf(stderr, "No attributes present \n");

  /* iterate through attributes */
  for(i=0;i<attr_count;i++){
     fprintf(stderr, "Attribute Number: %i\n",i);
     attribute = X509_REQ_get_attr(req,i);
     ASN1_TYPE *attr_type = X509_ATTRIBUTE_get0_type(attribute,i);
     int nid = OBJ_obj2nid(attribute->object);
     fprintf(stderr, "Attribute NID: %d , Name: %s \n",nid, OBJ_nid2ln(nid));
     fprintf(stderr, "First attribute field holds type %x: \n", attr_type);
int asn1type = attr_type ? ASN1_TYPE_get(attr_type) : 0;
fprintf(stderr, "Not dead yet\n");
     astring = (ASN1_PRINTABLESTRING *)X509_ATTRIBUTE_get0_data(attribute,0,asn1type,NULL);
fprintf(stderr, "Not dead yet\n");
if (!attr_type) {
fprintf(stderr, "%s\n", astring);
} else {
     for(j=0;j<astring->length;j++) fprintf (stderr,"%c", astring->data[j]);
}
     fprintf(stderr,"\n------------------------\n\n");
   } /* end for*/
}
...
@implementation AgentX509Certificate
...
@implementation AgentPKCS7Message
                                  ...
                                  signedAttributes:(NSDictionary *) signedAttributes
{
    ...
    return [[AgentPKCS7Message alloc] initWithPKCS7:p7];
}
...
+ (AgentPKCS7Message *) encryptedMessageWithCertificates:(NSArray *) certificates
    ...
    for (AgentX509Certificate *certificate in certificates) {
...
- (AgentX509Certificate *) signerCertificate {
    ...
    return [[AgentX509Certificate alloc] initWithX509:signer_cert];
}
...
@implementation AgentCertificateAuthority
void
                                            ...
                                            privateKey:(AgentEVPPKey *) caPrivateKey
{
    ...
    X509 *CAcert = caCertificate->cert;
    EVP_PKEY *CApkey = caPrivateKey->pkey;
    ...
    return [[AgentX509Certificate alloc] initWithX509:cert];
}
