;; test_bson.nu
;;  tests for AgentBSON representation.
;;
...
(class TestBSON is NuTestCase
     ...
     (- testRoundtrip is
        (set x (dict a:"a"
                     ...
                     h:nil))
        (set bson (AgentBSON bsonWithDictionary:x))
        (set data (bson dataRepresentation))
        (set bson2 (AgentBSON bsonWithData:data))
        ...
        ;; now stay as bson
        (set z (AgentBSON bsonWithDictionary:y))
     ...
     (- testOIDs is
        (10 times:
            (do (i)
                (set id1 ((AgentBSONObjectID objectID)))
                (set id2 ((AgentBSONObjectID alloc)
                          initWithData:(id1 dataRepresentation)))
                (assert_equal id1 id2)
                
                (set id3 (AgentBSONObjectID new))
                (set id4 (AgentBSONObjectID new))
     ...
     (- testCode is
        (set code '(do (x) (+ x 1)))
        (set bson (AgentBSON bsonWithDictionary:(dict code:code)))
