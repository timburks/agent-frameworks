...
+ (AgentBSONObjectID *) objectIDWithObjectIDPointer:(const bson_oid_t *) objectIDPointer;
/*! Raw object id */
- (bson_oid_t) oid;
...
void add_object_to_bson(bson *b, id key, id object)
{
    ...
    else if ([object isKindOfClass:[AgentBSONObjectID class]]) {
        bson_append_oid(b, name, [((AgentBSONObjectID *) object) objectIDPointer]);
    }
...
@implementation AgentBSONObjectID

+ (AgentBSONObjectID *) objectID
{
    bson_oid_t oid;
    bson_oid_gen(&oid);
    return [[AgentBSONObjectID alloc] initWithObjectIDPointer:&oid];
}

+ (AgentBSONObjectID *) objectIDWithData:(NSData *) data
{
    return [[AgentBSONObjectID alloc] initWithData:data];
}

+ (AgentBSONObjectID *) objectIDWithObjectIDPointer:(const bson_oid_t *) objectIDPointer
{
    return [[AgentBSONObjectID alloc] initWithObjectIDPointer:objectIDPointer];
}
...
@implementation AgentBSON

+ (AgentBSON *) bsonWithData:(NSData *) data
{
    return [[AgentBSON alloc] initWithData:data];
}
...
+ (NSMutableArray *) bsonArrayWithData:(NSData *) data
{
    ...
    while (bson_size(&bsonBuffer)) {
        ...
        AgentBSON *bsonObject = [[AgentBSON alloc] initWithBSON:bsonValue];
...
+ (AgentBSON *) bsonWithDictionary:(NSDictionary *) dictionary
{
    return [[AgentBSON alloc] initWithDictionary:dictionary];
}

+ (AgentBSON *) bsonWithList:(id) list
{
    return [[AgentBSON alloc] initWithList:list];
}
...
// When an unknown message is received by a AgentBSON object, treat it as a call to objectForKey:
    ...
    switch(bson_iterator_type(&it)) {
        ...
        case BSON_OBJECT:
            if (expandChildren) {
                ...
                value = [[AgentBSON alloc] initWithBSON:copied_bson];
            }
            break;
        case BSON_ARRAY:
...
bson *bson_for_object(id object)
{
    ...
    if ([object isKindOfClass:[AgentBSON class]]) {
        b = &(((AgentBSON *)object)->bsonValue);
    }
    else if ([object isKindOfClass:[NSDictionary class]]) {
        AgentBSON *bsonObject = [[AgentBSON alloc] initWithDictionary:object];
...
@implementation AgentBSONComparator

+ (AgentBSONComparator *) comparatorWithBSONSpecification:(AgentBSON *) s
{
    AgentBSONComparator *comparator = [[AgentBSONComparator alloc] init];
...
- (int) compareDataAtAddress:(void *) aptr withSize:(int) asiz withDataAtAddress:(void *) bptr withSize:(int) bsiz
{
    ...
    bsonA.data = aptr;
    //bsonA.owned = NO;
    AgentBSON *a = [[AgentBSON alloc] initWithBSON:bsonA];
    
    bson bsonB;
    bsonB.data = bptr;
    //bsonB.owned = NO;
    AgentBSON *b = [[AgentBSON alloc] initWithBSON:bsonB];
...
@implementation AgentBSONObjectSerialization

+ (id)BSONObjectWithData:(NSData *)data options:(AgentBSONReadingOptions)options error:(NSError **)error {

    return [[AgentBSON bsonWithData:data] dictionaryValue];
}

+ (NSData *)dataWithBSONObject:(id)object options:(AgentBSONWritingOptions)options error:(NSError **)error {
    return [[AgentBSON bsonWithDictionary:object] dataRepresentation];
}

@end

// convenience categories
@implementation NSData (AgentBSON)
- (NSMutableDictionary *) agent_BSONValue
{
    return [[AgentBSON bsonWithData:self] dictionaryValue];
}

@end

@implementation NSDictionary (AgentBSON)
- (NSData *) agent_BSONRepresentation
{
    return [[AgentBSON bsonWithDictionary:self] dataRepresentation];
}
