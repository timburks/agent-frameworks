...
AgentBSON *convertToBSONObject(id object) {
    if (!object) {
    ...
    if ([object isKindOfClass:[AgentBSON class]]) {
        ...
        return [[AgentBSON alloc] initWithDictionary:object];
    }
    else {
...
@interface AgentMongoDB ()
{
    mongo conn;
}
@end

@interface AgentMongoDBCursor ()
{
    mongo_cursor *cursor;
}
@end

@implementation AgentMongoDBCursor

- (AgentMongoDBCursor *) initWithCursor:(mongo_cursor *) c
{
    if (self = [super init]) {
        cursor = c;
    }
...
- (AgentBSON *) currentBSON
{
    bson copied_bson;
    bson_copy(&copied_bson, &(cursor->current));
    return [[AgentBSON alloc] initWithBSON:copied_bson];
}
...
- (id) insertObject:(id) insert intoCollection:(NSString *) collection
{
    if (![insert objectForKey:@"_id"]) {
        ...
        [insert setObject:[AgentBSONObjectID objectID] forKey:@"_id"];
    }
    if (enableUpdateTimestamps) {
        [insert setObject:[NSDate date] forKey:@"_up"];
    }
    AgentBSON *bsonObject = convertToBSONObject(insert);
    if (bsonObject) {
...
{
    AgentBSON *queryBSONObject = convertToBSONObject(query);
    bson bsonResult;
    ...
    [[[AgentBSON alloc] initWithBSON:bsonResult] dictionaryValue] : nil;
}

- (int) countWithCondition:(id) condition inCollection:(NSString *) collection inDatabase:(NSString *) database
{
    AgentBSON *conditionBSONObject = convertToBSONObject(condition);
...
{
    AgentBSON *keyBSONObject = convertToBSONObject(key);
    bson output;
...
- (AgentMongoDBCursor *) find:(id) query inCollection:(NSString *) collection
{
    AgentBSON *queryBSONObject = convertToBSONObject(query);
    mongo_cursor *cursor = mongo_find(&conn,
                                      ...
                                      &(queryBSONObject->bsonValue),
                                      0, 0, 0, 0 );
    return [[AgentMongoDBCursor alloc] initWithCursor:cursor];
}
...
- (AgentMongoDBCursor *) find:(id) query inCollection:(NSString *) collection returningFields:(id) fields numberToReturn:(int) nToReturn numberToSkip:(int) nToSkip
{
    AgentBSON *queryBSONObject = convertToBSONObject(query);
    AgentBSON *fieldsBSONObject = convertToBSONObject(fields);
    mongo_cursor *cursor = mongo_find(&conn,
                                      [collection cStringUsingEncoding:NSUTF8StringEncoding],
                                      &(queryBSONObject->bsonValue),
...
{
    AgentMongoDBCursor *cursor = [self find:query inCollection:collection];
...
{
    AgentMongoDBCursor *cursor = [self find:query inCollection:collection returningFields:fields numberToReturn:nToReturn numberToSkip:nToSkip];
        ...
        withCondition:(id) condition insertIfNecessary:(BOOL) insertIfNecessary updateMultipleEntries:(BOOL) updateMultipleEntries
{
    ...
    AgentBSON *updateBSONObject = convertToBSONObject(update);
    AgentBSON *conditionBSONObject = convertToBSONObject(condition);
...
{
    AgentBSON *commandBSONObject = convertToBSONObject(command);
    bson bsonResult;
    ...
    return (result == MONGO_OK) ? [[[AgentBSON alloc] initWithBSON:bsonResult] dictionaryValue] : nil;
}
