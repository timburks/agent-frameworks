...
@interface AgentHTTPService ()
@property (nonatomic, strong) AgentHTTPRequestRouter *router;
@property (nonatomic, strong) NSMutableDictionary *mimetypes;
@end

@implementation AgentHTTPService
@synthesize router, mimetypes;

+ (AgentHTTPService *) sharedService
{
    static dispatch_once_t once;
    static AgentHTTPService *sharedInstance;
    dispatch_once(&once, ^{
...
- (id) init {
    if (self = [super init]) {
        self.router = [[AgentHTTPRequestRouter alloc] init];
...
{
    [self.router insertHandler:[AgentHTTPRequestHandler handlerWithHTTPMethod:method path:path block:block]
                  level:0];
}

- (void) addHandlerWithPath:(NSString *) path directory:(NSString *) directory
{
    [self.router insertHandler:[AgentHTTPRequestHandler handlerWithPath:path directory:directory]
                  level:0];
}
