...
//
// AgentCocoaHTTPConnection
// A model for a web server's connection to a client.
// A AgentHTTPConnection manages the connection between the AgentHTTP server and a client.
// It is created by AgentHTTPServer instances in response to a new connection.
// The AgentHTTPConnection then receives data and constructs request objects to
// represent each complete request that it receives.
//
@interface AgentCocoaHTTPConnection : NSObject <AgentHTTPResponder>
...
- (id) initWithFileHandle:(NSFileHandle *) fileHandle server:(AgentHTTPServer *) server;
- (void) respondWithMessageData:(NSData *) messageData;

@end

@implementation NSFileHandle (AgentHTTP)
...
- (void) closeConnectionAndContinue:(AgentCocoaHTTPConnection *) connection;
- (void) addRequest:(id) request;
@end

@implementation AgentCocoaHTTPConnection
...
NSData *messageDataForResponse(AgentHTTPResponse *response)
{
...
@interface AgentCocoaHTTPServer ()
@property (nonatomic, strong) NSFileHandle *fileHandle;
...
@implementation AgentCocoaHTTPServer

#if !TARGET_OS_IPHONE
+ (void) load {
    //[AgentHTTPServer macros];
}
#endif
...
- (void)processNextRequest:(id)sender
{
    if ([self.requests count] > 0) {
		AgentHTTPRequest *request = [self.requests objectAtIndex:0];
        if (self.verbose) {
            NSLog(@"%@ %@\n%@",
                  request.method,
                  request.path,
...
- (void)processNewConnection:(NSNotification *)notification
{
    ...
    if (errorNo) {
        ...
        if (remoteFileHandle) {
            AgentCocoaHTTPConnection *connection =
            [[AgentCocoaHTTPConnection alloc]
